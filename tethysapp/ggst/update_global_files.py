...
CLSM_VARIABLES = ['time', 'lon', 'lat', 'SWE_inst',
                  'CanopInt_inst', 'TWS_inst',
                  'SoilMoist_P_inst']

# NOAH  variables of interest
NOAH_VARIABLES = ['time', 'lon', 'lat', 'SWE_inst', 'CanopInt_inst',
                  'SoilMoi0_10cm_inst', 'SoilMoi10_40cm_inst', 'SoilMoi40_100cm_inst',
                  'SoilMoi100_200cm_inst']

# VIC variables of interest
VIC_VARIABLES = ['time', 'lon', 'lat', 'SWE_inst', 'CanopInt_inst',
...
def get_catalog_urls(catalog_name, catalog):
    ...
    urls = [update_url(ds.access_urls['dap'])
...
def get_grace_urls(catalog):
    urls = [update_grace_url(ds.access_urls['file'])
...
def download_gldas_catalog(gldas_url, output_dir):
    ...
    valid_refs = [cat for cat in catalog.catalog_refs.keys() if '10_M' in cat if 'EP' not in cat]
    latest_versions = []
    ...
    output_paths = [download_catalog(output_dir, key, values) for key, values in url_dict.items()]
...
def gldas_concat_helper(grace_dir, model_dir, model_type, gldas_variables):
    ...
    sorted_files = sorted([(np.datetime64(datetime.strptime(re.search(r'\d{6}', _file).group(0), '%Y%m')), _file)
    ...
    if not os.path.exists(gldas_output_file):
        gldas_ds = xr.open_mfdataset(nc_files_glob, parallel=True).drop_vars('time_bnds')[gldas_variables]
        gldas_ds.to_netcdf(gldas_output_file)
    else:
        gldas_ds = xr.open_dataset(gldas_output_file)
        concat_list = list(np.array(final_files)[timesteps > gldas_ds.time.max().values])
        if len(concat_list) > 0:
            gldas_vic_concat = (xr.open_mfdataset(concat_list, parallel=True)
                .drop_vars('time_bnds')[gldas_variables])
...
def calculate_surface_area(ds):
    ...
    surface_area_array = (np.array([6371000 * math.radians(lat_interval_size)
                                    * 6371000 * math.radians(lon_interval_size)
...
def generate_gldas_global_files(grace_dir):
    def aggregate_gldas_anomalies(clsm_variables, noah_variables, vic_variables, variable_name='lwe_thickness'):
    ...
    print('GLOBAL SW')
    final_sw_ds = aggregate_gldas_anomalies(CLSM_SM_VARIABLES, NOAH_SW_VARIABLES, VIC_SW_VARIABLES)
    final_sw_ds.to_netcdf(os.path.join(grace_dir, 'GRC_sw.nc'))
    print('GLOBAL SM')
    final_sm_ds = aggregate_gldas_anomalies(CLSM_SM_VARIABLES, NOAH_SM_VARIABLES, VIC_SM_VARIABLES)
    final_sm_ds.to_netcdf(os.path.join(grace_dir, 'GRC_sm.nc'))
    print('GLOBAL SWE')
    final_swe_ds = aggregate_gldas_anomalies(CLSM_SWE_VARIABLES, NOAH_SWE_VARIABLES, VIC_SWE_VARIABLES)
    final_swe_ds.to_netcdf(os.path.join(grace_dir, 'GRC_swe.nc'))
    print('GLOBAL CANOPY')
    final_canopy_ds = aggregate_gldas_anomalies(CLSM_CANOPY_VARIABLES, NOAH_CANOPY_VARIABLES, VIC_CANOPY_VARIABLES)
    final_canopy_ds.to_netcdf(os.path.join(grace_dir, 'GRC_canopy.nc'))
    print('GLOBAL TWS')
    final_tws_ds = aggregate_gldas_anomalies(CLSM_TWS_VARIABLES, NOAH_TWS_VARIABLES, VIC_TWS_VARIABLES)
...
def generate_global_grace_nc(grace_dir):
    ...
    final_grace_ds = xr.Dataset({'lwe_thickness': gw_wsa_da, 'uncertainty': ds.uncertainty})
...
def generate_global_gw_nc(grace_dir):
    ...
    gw_uncertainty_da = (np.sqrt(np.abs(monthly_mean.uncertainty ** 2
...
def process_global_files(grace_dir, thredds_dir):
    ...
    gldas_dir = os.path.join(grace_dir, 'gldas')
    mascon_dir = os.path.join(grace_dir, 'mascon')
    download_gldas_catalog("https://hydro1.gesdisc.eosdis.nasa.gov/opendap/GLDAS/catalog.xml",
                           gldas_dir)
    download_grace_catalog('https://opendap.jpl.nasa.gov/opendap/hyrax/'
    ...
    grc_files = glob.glob(f"{grace_dir}GRC*.nc")
    [shutil.copy(_file, thredds_dir) for _file in grc_files]
    [clip_gloal_nc(_file, gpd.read_file(os.path.join(_dir.path, 'shape.geojson')), _dir.name, thredds_dir)
     for _file in glob.glob(os.path.join(thredds_dir, '*.nc'))
     for _dir in os.scandir(thredds_dir) if _dir.is_dir()]
